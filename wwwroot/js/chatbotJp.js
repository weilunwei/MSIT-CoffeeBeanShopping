...
$.ajax({
    ...
    error: function () {
        $('#chatbot-body').append('<div class="message-bot">データの読み込みに失敗しました。しばらく後に再度お試しください。</div>');
    }
});
scrollToBottom();
    ...
    `).join('');
        ...
        if (isUrl(data)) {
            ...
            setTimeout(() => {
                $('#chatbot-body').append(`
                    <div class="message-bot">
                        <a href="${resolvedUrl}" target="_self" class="btn btn-primary bg-success">チェックしてみましょう</a>
                    </div>
                `);
                scrollToBottom();
