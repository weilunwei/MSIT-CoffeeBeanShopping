...
{
	public class ProductServiceJp
	{
		private readonly ProjectContext _context;


		public ProductServiceJp(ProjectContext context)
		{
			_context = context;
		}
		// -----------------  獲取單個商品的資料
		public async Task<ProductJp?> Get_Product_By_IdAsync(int id)
		{
			return await _context.ProductJps.FirstOrDefaultAsync(p => p.Id == id);
		}
		//----------------- 拿到全部的 產地
		public async Task<List<string>> Get_All_Country_Async()
		{
			// 資料庫先抓
			var country = await _context.ProductJps
				.Select(c => c.Country)
				.ToListAsync();
		...
		{
			var all_baking = await _context.ProductJps
				.Select(b => b.Baking)
				.Distinct()
				.ToListAsync();
			return all_baking!;
		...
		{
			var flavor = await _context.ProductJps
				.Select(f => f.Flavor)
				.ToListAsync();
		...
		{
			var all_method = await _context.ProductJps
				.Select(b => b.Method)
				.Distinct()
				.ToListAsync();
			return all_method!;
		...
		{
			var all_timelimit = await _context.ProductJps
		...
		{
			var fragrance = await _context.ProductJps
		...
		{
			var sour = await _context.ProductJps
				.Where(c => c.Id == id)
				.Select(b => b.Sour)
				.FirstOrDefaultAsync();
			return (int)sour;
		...
		{
			var bitter = await _context.ProductJps
				.Where(c => c.Id == id)
				.Select(b => b.Bitter)
				.FirstOrDefaultAsync();
			return (int)bitter;
		...
		{
			var sweet = await _context.ProductJps
				.Where(c => c.Id == id)
				.Select(b => b.Sweet)
				.FirstOrDefaultAsync();
			return (int)sweet;
		...
		{
			var strong = await _context.ProductJps
				.Where(c => c.Id == id)
				.Select(b => b.Strong)
				.FirstOrDefaultAsync();
			return (int)strong;
